		...
		elseif($role == "information_officer")
		{
			...
			if($rows != 0)
			{
				...
				if($email == $db_email && $password ==$db_password){
					...
					@$_SESSION['user_email']=$email;
					@$_SESSION['user_id']=$db_user_id;
					header("Location:info_off_dashboard.html");		//TODO: change to absolute path of the html file
				}
				else {
					$wrongpass="Wrong Password";
				}
		...
		elseif($role == "admin")
		{
			...
			if($rows != 0)
			{
				...
				if($email == $db_email && $password ==$db_password){
					...
					@$_SESSION['user_email']=$email;
					@$_SESSION['user_id']=$db_user_id;
					header("Location:admin_dashboard.html");		//TODO: change to absolute path of the html file
				}
				else {
					$wrongpass="Wrong Password";
				}
